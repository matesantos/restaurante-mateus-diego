...
public class InterfaceGui {
	...
	private void MenuPrincipal() {
		...
		while (true) {
			int opcao = new Integer(JOptionPane.showInputDialog("Escolhe a opção:\n1-Gerenciar Gerente\n2-Gerenciar Funcionário\n3-Gerenciar Cliente"
					+ "											\n4-Saber o número de funcionários no restaurante \n5-Enviar Promocao \n6-Sair"));
			...
			case 4:
				getNumberTotalEmployed();
			break;
			
			case 5:
				MensagemMediator mediador = new MensagemMediator();
				 
			    ClaroColleague claro = new ClaroColleague(mediador);
			    OiColleague oi = new OiColleague(mediador);
			 
			    mediador.adicionarColleague(claro);
			    mediador.adicionarColleague(oi);
			     
			    claro.enviarMensagem("Enviando promoção para usuário Claro!");
			    
			    oi.enviarMensagem("Enviando promoção para usuário Oi!");
			break;
	...
	private void getNumberTotalEmployed(){
		
		int numGerente = 0;
		int numFuncionario = 0;
		
		for (Gerente g: DaoGerente.getGerenteList()) 
			numGerente += g.getCount(); 
		
		
		for(Funcionario f: DaoFuncionario.getFuncionarioList())
			numFuncionario = f.getCount();
		
		JOptionPane.showMessageDialog(null, "Número total de funcionários são: "+ numFuncionario + numGerente);
